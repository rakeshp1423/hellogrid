...
export default function Canvas({ components, setComponents, selectedIds, setSelectedIds }) {
  ...
  const [{ isOver }, drop] = useDrop(() => ({
    ...
    drop: (item, monitor) => {
      ...
      const newComponent = {
        id: Date.now(),
        type: item.type,
        x, y, width: 200, height: 100, content: "Edit me",
        locked: false, visible: true,
        styles: {
          fontSize: 16, fontFamily: "Arial", color: "#000000",
          backgroundColor: "#ffffff", padding: 0, margin: 0,
          fontWeight: "normal", textAlign: "left", borderRadius: 4,
        },
      };
  ...
  const updateComponent = (id, newProps) => {
    setComponents(prev => prev.map(el => el.id === id ? { ...el, ...newProps } : el));
  };
  ...
  const handleGroupDrag = (id, deltaX, deltaY) => {
    setComponents(prev => prev.map(el => selectedIds.includes(el.id) ? { ...el, x: el.x + deltaX, y: el.y + deltaY } : el));
  };
  ...
  const handleContextAction = action => {
    ...
    switch (action) {
      ...
      case "Duplicate":
        ...
        const copy = { ...original, id: Date.now(), x: original.x + 20, y: original.y + 20 };
      ...
      case "Edit Text":
        ...
        if (content !== null) updateComponent(id, { content });
        break;
      default: break;
    }
  ...
  return (
    <main className="flex-1 relative bg-[#1b1b1e] rounded p-4 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={showGrid} onChange={() => setShowGrid(prev => !prev)} className="sr-only peer" />
              <div className="w-11 h-6 bg-gray-700 rounded-full peer-checked:bg-indigo-600 transition-all"></div>
      ...
      <div
        ...
        className={`w-full h-full rounded-xl border border-dashed relative ${isOver ? "border-indigo-500" : "border-gray-700"}`}
      >
        {showGrid && <div className="absolute inset-0 pointer-events-none z-0">
          {Array.from({ length: 100 }).map((_, i) => <div key={`v-${i}`} className="absolute top-0 bottom-0 border-l border-gray-700 opacity-20" style={{ left: `${i * gridSize}px` }} />)}
          {Array.from({ length: 100 }).map((_, i) => <div key={`h-${i}`} className="absolute left-0 right-0 border-t border-gray-700 opacity-20" style={{ top: `${i * gridSize}px` }} />)}
        </div>}
        ...
        {components.filter(c => c.visible !== false).map(c => (
          <Rnd
            ...
            onDragStop={(_, d) => {
              ...
              selectedIds.includes(c.id) ? handleGroupDrag(c.id, deltaX, deltaY) : updateComponent(c.id, { x: newX, y: newY });
            }}
            ...
            className={`rounded shadow text-sm flex items-center justify-center overflow-hidden transition border ${selectedIds.includes(c.id) ? "border-indigo-400" : "border-gray-500"}`}
            style={{
          ...
          >{c.content || c.type.toUpperCase()}</Rnd>
        ))}

        <ContextMenu x={contextMenu.x} y={contextMenu.y} show={contextMenu.show} onClose={() => setContextMenu({ ...contextMenu, show: false })} onAction={handleContextAction} />
      </div>
    </main>
  );
}
